"""pytest configuration for the SEUR test suite."""
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.seur`` loadable from config-flow / setup tests."""
    yield
...
def reset_one_shot_warnings():
    ...
    ``parcels._unmapped_statuses_logged`` and ``_payload_shapes_logged`` are
    module-level, so without this a test's one-shot-warning assertion would
    depend on which test already triggered that warning first.
    """
    from custom_components.seur import parcels

    parcels._unmapped_statuses_logged.clear()
    parcels._payload_shapes_logged.clear()
    yield

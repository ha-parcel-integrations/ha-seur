"""Tests for SEUR sensor property logic."""

from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.seur.const import ParcelStatus
from custom_components.seur.sensor import (
    SEURAwaitingPickupSensor,
    SEURDeliveredParcelsSensor,
    SEURIncomingParcelsSensor,
    SEURLastUpdateSensor,
    SEURNextDeliverySensor,
    SEURParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = SEURIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = SEURParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = SEURParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = SEURNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = SEURNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = SEURDeliveredParcelsSensor(coordinator, _entry())
...
def test_awaiting_pickup_requires_pickup_and_at_pickup_point():
    coordinator = _coordinator(
        [
            ...
            _parcel("HOME", status=ParcelStatus.AT_PICKUP_POINT),
        ]
    )
...
def test_last_update_sensor():
    ...
    sensor = SEURLastUpdateSensor(coordinator, _entry())

"""Tests for the SEUR deliveries calendar."""

from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.seur.calendar import SEURDeliveriesCalendar
...
def _parcel(
    barcode, planned_from=None, planned_to=None, pickup=False, pickup_point=None
):
    return {
        "barcode": barcode,
...
def _cal(data):
    return SEURDeliveriesCalendar(_coordinator(data), _entry())
    ...
    cal = _cal(
        [
            _parcel(
                ...
                planned_from="2099-01-01T10:00:00Z",
                pickup=True,
                pickup_point="SEUR Shop",
            )
        ]
    )
    assert cal._events()[0].location == "SEUR Shop"

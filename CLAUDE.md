...
Home Assistant custom integration for **SEUR** parcel tracking.
...
private `carrier-research/seur/api/` and are **never** copied here.
...
SEUR is an account-inbox integration. A credential rejection starts reauth;
all other API failures remain retryable. Runtime-only authentication state and
...
Home Assistant (`SEURAuthError` from any per-parcel fetch raises
...
refresh. `seur/coordinator.py` is the canonical implementation
...
```
python -m pytest tests/ --cov=custom_components.seur
```

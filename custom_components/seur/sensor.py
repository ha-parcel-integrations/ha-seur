"""Sensor platform for the SEUR parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: SEURConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up SEUR sensor entities from a config entry."""
    ...
    current_barcodes: set[str] = {p.get("barcode", "") for p in coordinator.data or []}
    ...
    entities: list[SensorEntity] = [
        SEURIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    entities.extend(
        [
            SEUROutgoingParcelsSensor(coordinator, entry),
            SEUROutgoingDeliveredParcelsSensor(coordinator, entry),
            SEURAwaitingPickupSensor(coordinator, entry),
        ]
    )
    for parcel in coordinator.data or []:
        entities.append(SEURParcelSensor(coordinator, entry, parcel.get("barcode", "")))
    entities.append(SEURNextDeliverySensor(coordinator, entry))
    entities.append(SEURDeliveredParcelsSensor(coordinator, entry))
    entities.append(SEURLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class SEURIncomingParcelsSensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                SEURParcelSensor(self.coordinator, self._entry, barcode)
...
class SEURParcelSensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked SEUR parcel."""
    ...
    def __init__(
        self, coordinator: SEURCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class SEUROutgoingParcelsSensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
    """Summary of active outgoing parcels."""
    ...
    def __init__(self, coordinator: SEURCoordinator, entry: ConfigEntry) -> None:
        """Initialise the outgoing summary."""
        super().__init__(coordinator)
    ...
    @property
    def native_value(self) -> int:
        """Return active outgoing parcel count."""
        return len(self.coordinator.outgoing)

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return outgoing parcels as an unrecorded attribute."""
        return {"parcels": self.coordinator.outgoing}


class SEUROutgoingDeliveredParcelsSensor(SEUROutgoingParcelsSensor):
    """Summary of recently delivered outgoing parcels."""

    _attr_translation_key = "outgoing_delivered_parcels"

    def __init__(self, coordinator: SEURCoordinator, entry: ConfigEntry) -> None:
        """Initialise the delivered-outgoing summary."""
        super().__init__(coordinator, entry)
        self._attr_unique_id = f"{entry.entry_id}_outgoing_delivered_parcels"

    @property
    def native_value(self) -> int:
        """Return delivered outgoing parcel count."""
        return len(self.coordinator.delivered_outgoing)

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return delivered outgoing parcels as an unrecorded attribute."""
        return {"parcels": self.coordinator.delivered_outgoing}


class SEURAwaitingPickupSensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
    """Active incoming parcels waiting at a pickup point."""
    ...
    def __init__(self, coordinator: SEURCoordinator, entry: ConfigEntry) -> None:
        """Initialise the pickup-point parcel summary."""
        super().__init__(coordinator)
    ...
    @property
    def native_value(self) -> int:
        """Return pickup-point parcel count."""
        return len(self._get_pickup_parcels())

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return pickup-point parcels as an unrecorded attribute."""
        return {"parcels": self._get_pickup_parcels()}


class SEURNextDeliverySensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
...
class SEURDeliveredParcelsSensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
    """Recently delivered tracked SEUR parcels."""
...
class SEURLastUpdateSensor(CoordinatorEntity[SEURCoordinator], SensorEntity):
    """Diagnostic sensor reporting when SEUR was last polled successfully."""

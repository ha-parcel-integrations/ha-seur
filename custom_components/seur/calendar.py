"""Calendar platform for the SEUR parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the SEUR deliveries calendar from a config entry."""
    async_add_entities([SEURDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class SEURDeliveriesCalendar(CoordinatorEntity[SEURCoordinator], CalendarEntity):
    """A read-only calendar of expected SEUR deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "SEUR parcel")
            description_parts = [
